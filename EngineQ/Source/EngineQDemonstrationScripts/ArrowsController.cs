		...
		protected override void OnUpdate()
		{
			if (move || rotate)
			{
				...
				if (Utils.RayPlaneIntersection(new Plane(this.Transform.GlobalPosition, planeNormal), ray, out distance))
				{
					...
					if (rotate)
					{
						this.Transform.GlobalRotation = Quaternion.CreateFromAxisAngle(arrowDir, Vector3.DotProduct(diff, localUp)) * this.initialRotation;

						if (transform != null)
							transform.GlobalRotation = this.Transform.GlobalRotation;
					}
